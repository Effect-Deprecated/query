...
export const effect = (() => {
  const f = <E, A>(
    name: string,
    self: () => Effect.Effect<TestServices.TestServices | TestConsole.TestConsole, E, A>,
    timeout = 5_000
  ) => {
    return it(
      name,
      () =>
        pipe(
          Effect.suspend(self),
          Effect.provideLayer(Layer.merge(TestEnvironment.testContext(), TestConsole.layer)),
          Effect.runPromise
        ),
      timeout
    )
...
export const scoped = <E, A>(
  name: string,
  self: () => Effect.Effect<Scope.Scope | TestServices.TestServices | TestConsole.TestConsole, E, A>,
  timeout = 5_000
) => {
  return it(
    name,
