...
export const UserDataSource: DataSource.DataSource<never, UserRequest> = DataSource
...
export const getAllUserIds: Query.Query<never, never, ReadonlyArray<number>> = Query.fromRequest(
  GetAllIds({}),
  UserDataSource
)

export const getUserNameById = (id: number): Query.Query<never, never, string> =>
  Query.fromRequest(GetNameById({ id }), UserDataSource)

export const getAllUserNames: Query.Query<never, never, ReadonlyArray<string>> = pipe(
  getAllUserIds,
